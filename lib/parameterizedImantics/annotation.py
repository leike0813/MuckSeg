...
class Annotation(Semantic):
    ...
    @classmethod
    def from_mask(cls, mask, image=None, category=None, config=_default_config):
        """
        ...
        """
        return cls(image=image, category=category, mask=mask, config=config)
    
    @classmethod
    def from_bbox(cls, bbox, image=None, category=None, config=_default_config):
        """
        ...
        """
        return cls(image=image, category=category, bbox=bbox, config=config)
    
    @classmethod
    def from_polygons(cls, polygons, image=None, category=None, config=_default_config):
        """
        ...
        *No sepcificaiton is reqiured between which
        ...
        """
        return cls(image=image, category=category, polygons=polygons, config=config)

    def __init__(self, image=None, category=None, bbox=None, mask=None, polygons=None, id=0,
                 color=None, metadata={}, width=0, height=0, config=_default_config):
        ...
        self.color = Color.create(color, config=config)
        self._c_bbox = BBox.create(bbox, config=config)
        self._c_mask = Mask.create(mask, config=config)
        self._c_polygons = Polygons.create(polygons, config=config)
        ...
        super(Annotation, self).__init__(id, metadata, config)
    ...
    def area(self):
        ...
        elif self._init_with_polygons: # 20230608: Modified by Joshua Reed
    ...
    def index(self, image):
        ...
        #     self.id += 1
            # self.index(image) # recursive call may cause miscellaneous errors
        # else:
        ...
        # 20230608: modified by Joshua reed
        while True:
            found = annotation_index.get(self.id)
            if not found:
                annotation_index[self.id] = self
    ...
    def save(self, file_path, style=None):
...
class BBox:
    ...
    @classmethod
    def from_mask(cls, mask, config=_default_config):
        """
        ...
        """
        return Mask.create(mask, config=config).bbox()
    
    @classmethod
    def from_polygons(cls, polygons, config=_default_config):
        """
        ...
        :returns: :class:`BBox` repersentation
        """
        return Polygons.create(polygons, config=config).bbox()
    
    @classmethod
    def create(cls, bbox, style=None, config=_default_config):
        """
        Creates :class:`BBox`
        ...
        if isinstance(bbox, BBox.INSTANCE_TYPES):
            return BBox(bbox, style=style, config=config)
    ...
    @classmethod
    def empty(cls, config=_default_config):
        """
        :returns: Empty :class:`BBox` object
        """
        return BBox((0, 0, 0, 0), config=config)

    _c_polygons = None
    _c_mask = None

    def __init__(self, bbox, style=None, config=_default_config):
    ...
    def polygons(self):
        ...
        if not self._c_polygons:
            ...
            return Polygons([polygon], config=self.config)
        return self._c_polygons
    ...
    def mask(self, width=None, height=None):
        ...
        if not self._c_mask:
            ...
            self._c_mask = Mask(mask, config=self.config)
    ...
    def draw(self, image, color=None, thickness=None):
        ...
        *This function modifies the image array*
        ...
        """
        if not thickness:
            thickness = self.config.IMAGE.DRAW.DEFAULT_BBOX_THICKNESS
        color = Color.create(color, config=self.config).rgb
    ...
    def __eq__(self, other):
        if isinstance(other, self.INSTANCE_TYPES):
            other = BBox(other, config=self.config)
...
class Polygons:
    ...
    def from_mask(cls, mask, config=_default_config):
        ...
        """
        return Mask.create(mask, config=config).polygons()

    @classmethod
    def from_bbox(cls, bbox, style=None, config=_default_config):
        """
        ...
        :returns: :class:`Polygons` repersentation
        """
        return BBox.create(bbox, config=config).polygons()
        
    @classmethod
    def create(cls, polygons, config=_default_config):

        if isinstance(polygons, Polygons.INSTANCE_TYPES):
            return Polygons(polygons, config=config)
    ...
    def bbox(self):
        ...
        if not self._c_bbox:
            ...
            self._c_bbox = BBox((x_min, y_min, x_max, y_max), config=self.config)
    ...
    @property
    def simplified_polygons(self):
        """
        20230608: Feature added by Joshua Reed
        :return:
        """
        if not self._c_simplified_polygons:
            polygons = [cv2.approxPolyDP(
                polygon.reshape(-1, 1, 2),
                self.config.ANNOTATION.POLYGON.EXPORT_SIMPLIFY_EPSILON,
                True
            ) for polygon in self.polygons]
            self._c_simplified_polygons = [polygon.flatten() for polygon in polygons]
        return self._c_simplified_polygons
    ...
    def segmentation(self):
        ...
        if not self._c_segmentation:
            if self.config.ANNOTATION.POLYGON.EXPORT_SIMPLIFIED: # 20230608: Feature added by Joshua Reed
                self._c_segmentation = [polygon.tolist() for polygon in self.simplified_polygons]
            else:
    ...
    def draw(self, image, color=None, thickness=None):
        ...
        *This function modifies the image array*
        ...
        """
        if not thickness:
            thickness = self.config.IMAGE.DRAW.DEFAULT_POLYGON_THICKNESS
        color = Color.create(color, config=self.config).rgb
        image_copy = image.copy()
    ...
    def __eq__(self, other):
        if isinstance(other, self.INSTANCE_TYPES):
            other = Polygons(other, config=self.config)
...
class Mask:
    ...
    @classmethod
    def from_polygons(cls, polygons, config=_default_config):
        return Polygons.create(polygons, config=config).mask()
    
    @classmethod
    def from_bbox(cls, bbox, config=_default_config):
        return BBox.create(bbox, config=config).mask()
            
    @classmethod
    def create(cls, mask, config=_default_config):
        if isinstance(mask, Mask.INSTANCE_TYPES):
            return Mask(mask, config=config)
    ...
    def __init__(self, array, config=_default_config):
    ...
    def bbox(self):
        ...
        if not self._c_bbox:
            ...
            if not np.any(rows) or not np.any(cols):
                return BBox.empty(config=self.config)

            rmin, rmax = np.where(rows)[0][[0, -1]]
            cmin, cmax = np.where(cols)[0][[0, -1]]

            self._c_bbox = BBox((cmin, rmin, cmax, rmax), config=self.config)
    ...
    def polygons(self):
        ...
        :rtype: :class:`Polygons`
        """
        if not self._c_polygons: # 20230608: Feature added by Joshua Reed
            if self.config.ANNOTATION.MASK.POLYGON_APPROX_METHOD.lower() == 'none':
                poly_approx_method = cv2.CHAIN_APPROX_NONE
            elif self.config.ANNOTATION.MASK.POLYGON_APPROX_METHOD.lower() == 'simple':
                poly_approx_method = cv2.CHAIN_APPROX_SIMPLE
            elif self.config.ANNOTATION.MASK.POLYGON_APPROX_METHOD.lower() == 'l1':
                poly_approx_method = cv2.CHAIN_APPROX_TC89_L1
            elif self.config.ANNOTATION.MASK.POLYGON_APPROX_METHOD.lower() == 'kcos':
                poly_approx_method = cv2.CHAIN_APPROX_TC89_KCOS
            else:
                raise RuntimeError('Invalid polygon approximation method: {fmt}'.format(
                    fmt=self.config.ANNOTATION.MASK.POLYGON_APPROX_METHOD))

            # Generate polygons from mask
            mask = self.array.astype(np.uint8)
            mask = cv2.copyMakeBorder(mask, 1, 1, 1, 1, cv2.BORDER_CONSTANT, value=0)
            polygons = cv2.findContours(mask, cv2.RETR_LIST, poly_approx_method, offset=(-1, -1))
            polygons = polygons[0] if len(polygons) == 2 else polygons[1]
            polygons = [polygon.flatten() for polygon in polygons]

            self._c_polygons = Polygons(polygons, config=self.config)
    ...
    def union(self, other):
        ...
        if isinstance(other, np.ndarray):
            other = Mask(other, config=self.config)
        
        return Mask(np.logical_or(self.array, other.array), config=self.config)
    ...
    def intersect(self, other):
        ...
        return Mask(np.logical_and(self.array, other.array), config=self.config)
    ...
    def invert(self):
        ...
        :return: resulting :class:`Mask`
        """        
        return Mask(np.invert(self.array), config=self.config)
    
    def __invert__(self):
        return self.invert()
    
    def draw(self, image, color=None, alpha=None):
        """
        ...
        *This function modifies the image array*
        ...
        """
        if not alpha:
            alpha = self.config.IMAGE.DRAW.DEFAULT_MASK_ALPHA
        color = Color.create(color, config=self.config).rgb
        image_copy = image.copy()
        for c in range(3):
    ...
    def contains(self, item):
        ...
        if isinstance(item, np.ndarray):
            item = Mask(item, config=self.config)

        if isinstance(item, Mask):
            return self.intersect(item).area() > 0 # TODO:Check
    ...
    def match(self, item, threshold=None):
        """
        ...
        """
        if not threshold:
            threshold = self.config.ANNOTATION.MASK.DEFAULT_MATCH_THRESHOLD
