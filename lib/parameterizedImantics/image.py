...
from .utils import json_default, replace_absolute_image_path
...
class Image(Semantic):
    ...
    @classmethod
    def from_folder(cls, directory, config=_default_config):
        """
        ...
        for path, _, files in os.walk(directory):
            for name in files:
                ...
                if file_path.lower().endswith(cls.FORMATS):
                    images.append(cls.from_path(file_path, config=config))
        
        return images

    @classmethod
    def from_path(cls, path, config=_default_config):
        """
    ...
    @classmethod
    def from_coco(cls, coco, dataset=None, config=_default_config):
        """
    ...
    @classmethod
    def empty(cls, width=0, height=0, config=_default_config):
        """
        Creates an empty :class:`Image`
        """
        return cls(width=width, height=height, config=config)

    # annotations = {} # 20230616: use class properties is meaningless, Modified by Joshua Reed
    # categories = {}

    def __init__(self, image_array=None, annotations=[], path="",
                 id=0, metadata={}, dataset=None, width=0, height=0,
        ...
        super(Image, self).__init__(id, metadata, config)
    ...
    def add(self, annotation, category=None):
        ...
        if isinstance(annotation, Mask):
            ...
            if width == self.width and height == self.height:
                annotation = Annotation.from_mask(annotation, image=self, category=category, config=self.config)
            else:
        ...
        if isinstance(annotation, BBox):
            annotation = Annotation.from_bbox(annotation, image=self, category=category, config=self.config)

        if isinstance(annotation, Polygons):
            annotation = Annotation.from_polygons(annotation, image=self, category=category, config=self.config)
    ...
    def index(self, dataset):
        ...
        #     self.path = replace_absolute_image_path(self.path, dataset.info['image_root'], self.config.IMAGE.IMAGE_ROOT)

        for annotation in self.iter_annotations():
            annotation.index(dataset)

    def draw(self, bbox=None, outline=None, mask=None, text=None, thickness=None,
             alpha=None, categories=None, text_scale=None, color_by_category=None):
        """
        ...
        """
        if not bbox:
            bbox = self.config.IMAGE.DRAW.DEFAULT_DRAW_BBOX
        if not outline:
            outline = self.config.IMAGE.DRAW.DEFAULT_DRAW_OUTLINE
        if not mask:
            mask = self.config.IMAGE.DRAW.DEFAULT_DRAW_MASK
        if not alpha:
            alpha = self.config.IMAGE.DRAW.DEFAULT_MASK_ALPHA
        if not text:
            text = self.config.IMAGE.DRAW.DEFAULT_DRAW_TEXT
        if not text_scale:
            text_scale = self.config.IMAGE.DRAW.DEFAULT_COLOR_BY_CATEGORY
        ...
        for annotation in self.iter_annotations():
            ...
            if (categories is None) or (category in categories):
                ...
                if outline:
                    annotation.polygons.draw(
                        temp_image, color=color,
                        thickness=thickness if thickness else self.config.IMAGE.DRAW.DEFAULT_OUTLINE_THICKNESS
                    )

                if bbox:
                    annotation.bbox.draw(
                        temp_image, color=color,
                        thickness=thickness if thickness else self.config.IMAGE.DRAW.DEFAULT_BBOX_THICKNESS
                    )
                if text:
